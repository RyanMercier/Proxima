...
An honest validator with partial observation misses at most $k_{\max}=2$ transactions; this is the partial-observation model of Sect.~\ref{sec:model}, and it is what the calibration of Sect.~\ref{sec:calibration} samples. The threshold comes from that calibration: the 99th percentile of the sampled honest distance distribution is approximately $4.1$, and the $1.2$ safety margin gives $\tau = 1.2 \cdot 4.1 \approx 4.9$. Measured over the same calibration sample, the fraction of honest draws whose distance exceeds $4.9$ is at most $0.005$; this is the exclusion probability conditioned on having an incomplete view. Exclusion requires both an incomplete view, which occurs with probability $p_{\mathrm{miss}} = 0.37$ (Sect.~\ref{sec:model}), and a conditional exceedance, so the unconditional per-validator exclusion probability is at most $p = 0.37 \cdot 0.005 = 0.00185$.

Let $X$ be the number of honest validators excluded. $X$ is a sum of $N$ independent indicators bounded in $[0,1]$ with mean $\mu \le pN$. Liveness fails only if $X \ge N/3$. Since $p \ll 1/3$, applying Hoeffding's inequality with $t = 1/3 - p = 1/3 - 0.00185 \approx 0.33$, so that $2t^2 \approx 0.22$:
\begin{equation*}
...
This section applies the primitive to the consensus path itself. The resulting protocol, Proxima, runs two phases: Phase~1 measures agreement geometrically and repairs stragglers, and Phase~2 converts agreement into finality through aggregated BLS commitments; when enough validators already hold the proposed block, the two collapse into a single round trip. Sect.~\ref{sec:flat} presents the flat protocol, Sect.~\ref{sec:tree} routes the same two phases through a tree of small groups, Sect.~\ref{sec:why-phase2} explains why the signature phase can be accelerated but never skipped, and Sect.~\ref{sec:eth-committees} contrasts the resulting group structure with Ethereum's committees.
...
Table~\ref{tab:cross-shard} compares the three methods at 1000 cross-shard transactions per pair, 100 validators per shard, and 95\% pre-deadline propagation.
...
\textbf{Optimistic fast path.} When all validators have complete state, every Phase 1 message carries a matching commitment. The aggregator collects the commitments in round one and issues the finality certificate immediately (Sect.~\ref{sec:flat}). HotStuff still runs 3 rounds because it cannot measure agreement quality before votes are counted. The fast-path probability is approximately $(1 - p_{\mathrm{miss}})^N$; since this is the probability that every honest validator matches the proposal, while the certificate requires only $2N/3$ matching commitments, it is a lower bound on fast-path engagement. At $p_{\mathrm{miss}} = 0.05$ and $N = 10$ honest validators (the leaf-group regime) it is $\approx 60\%$, rising to $\approx 90\%$ at $p_{\mathrm{miss}} = 0.01$.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.7\textwidth]{fig4_fast_path.png}
  \caption{Lower bound on fast-path probability, $(1 - p_{\mathrm{miss}})^N$, as a function of partial-observation rate and honest validator count; engagement requires only $2N/3$ matching commitments. The fast path dominates on good networks and decays with packet loss.}
  \label{fig:fast-path}
\end{figure}
...
The fast-path certificate is itself a set of at least $2N/3$ signed commitments on $H(B)$, collected in Phase 1 rather than Phase 2, so Theorem~\ref{thm:safety} applies to it verbatim: an aggregator cannot fabricate a fast-path certificate, and a Byzantine aggregator can at most withhold one, delaying the round, which is a liveness effect.
...
Proxima is implemented in Python as a working BFT blockchain with account-based state, an HTTP node API, an interactive demo, benchmarks, and the evaluation figures, all reproducible from the repository~\cite{proxima-tr}. Digest computation costs one SHA-512 per transaction: the 64-byte output is split into eight 8-byte segments, each reduced modulo $10^4$ into a coordinate in $[0,1)$, and a validator's digest is the vectorized sum of its transaction vectors, traveling as 64 bytes on the wire. BLS signatures run through two interchangeable code paths: py-ecc~\cite{pyecc} performs real BLS12-381 operations in the live demo, while large-scale benchmarks substitute hash-based mocks that preserve the 96-byte signature and 48-byte public-key wire sizes; we cross-validated the paths by running the small-$N$ demo under both and confirming identical per-block message totals. All message counts are tracked by incrementing a counter on each simulated send, using identical byte constants for Proxima, HotStuff, and PBFT, and the latency projections use published blst~\cite{blst} microbenchmark constants (Sect.~\ref{sec:eval}). On the aggregation side, the implementation computes the reference digest once per block, excludes Byzantine validators before any signature work, synchronizes stragglers with a single bloom-diff push rather than request-response retransmission, aggregates the speculative Phase 1 commitments directly into the fast-path certificate, and in tree mode splits aggregation so that each leaf leader combines only a branching factor's worth of signatures.
